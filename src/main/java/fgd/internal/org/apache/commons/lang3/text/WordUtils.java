 ...
 */
package fgd.internal.org.apache.commons.lang3.text;
...
public class WordUtils {
    ...
    public static boolean containsAllWords(CharSequence word, CharSequence... words) {
        if (isEmpty(word) || null == words || 0 == words.length /*ArrayUtils.isEmpty(words)*/) {
            return false;
        }
    ...
    private static boolean isBlank(CharSequence w) {
		return null == w || 0 == w.toString().trim().length();
	}

	private static boolean isEmpty(CharSequence word) {
		return null == word || isEmpty(word.toString());
	}
